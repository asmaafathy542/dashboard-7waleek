...
const STATUS_COLORS = {
    active: { bg: "var(--success-bg)", color: "var(--success)", border: "var(--success-bg)" },
    inactive: { bg: "var(--danger-bg)", color: "var(--danger)", border: "var(--danger-bg)" },
    suspended: { bg: "var(--warning-bg)", color: "var(--warning)", border: "var(--warning-bg)" },
};
        ...
        >
            ...
            >
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                    <button onClick={onCancel} style={{ flex: 1, padding: "10px", borderRadius: "8px", border: "1px solid #e4e2dd", background: "var(--bg-card)", fontSize: "13px", fontWeight: 500, cursor: "pointer", color: "var(--text-sub)" }}>
                        Cancel
                    </button>
                    <button onClick={onConfirm} style={{ flex: 1, padding: "10px", borderRadius: "8px", border: "none", background: danger ? "var(--danger)" : "var(--text-main)", color: "var(--bg-card)", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>
                        {confirmLabel}
                    </button>
                </div>
            </div>
...
function CreatePlaceModal({ onClose, onCreated }) {
    ...
    const inputStyle = { width: "100%", padding: "9px 12px", borderRadius: "8px", border: "1px solid #e4e2dd", fontSize: "13px", color: "var(--text-main)", outline: "none", fontFamily: "inherit", boxSizing: "border-box", background: "var(--bg-card)" };
    const labelStyle = { fontSize: "12px", fontWeight: 600, color: "var(--text-sub)", marginBottom: "5px", display: "block" };
    ...
    return (
        <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--bg-card)", borderRadius: "16px", width: "100%", maxWidth: "520px", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
                <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #f1f0ec", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                        <h2 style={{ fontSize: "1rem", fontWeight: 700, color: "var(--text-main)", margin: 0 }}>🏪 Create New Place</h2>
                        <p style={{ fontSize: "12px", color: "var(--icon-muted)", margin: "3px 0 0" }}>Creates a place and its owner account automatically</p>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                        ...
                        <div style={{ gridColumn: "1 / -1" }}>
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "5px" }}>
                                ...
                                <button type="button" onClick={handleUseMyLocation} disabled={geoLoading} style={{ fontSize: "11px", fontWeight: 600, padding: "4px 10px", borderRadius: "6px", border: "1px solid #bfdbfe", background: geoLoading ? "var(--bg-surface)" : "var(--info-bg)", color: geoLoading ? "var(--icon-muted)" : "var(--color-primary-hover)", cursor: geoLoading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: "4px" }}>
                            ...
                            {form.location_link && !form.latitude && (
                                <div style={{ marginTop: "5px", fontSize: "11px", color: "var(--icon-muted)" }}>
                        ...
                        <div>
                            <label style={labelStyle}>
                                Latitude * <span style={{ fontWeight: 400, color: "var(--icon-muted)" }}>(22 → 32 لمصر)</span>
                            </label>
                            <input type="number" step="any" placeholder="e.g. 30.0444" value={form.latitude} onChange={handleChange("latitude")} style={inputStyle} />
                        </div>
                        <div>
                            <label style={labelStyle}>
                                Longitude * <span style={{ fontWeight: 400, color: "var(--icon-muted)" }}>(24 → 37 لمصر)</span>
                            </label>
    ...
    <button
        ...
        style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: "4px", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--icon-muted)" }}
    >
        {showPass ? (
            // Eye-off icon
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <circle cx="12" cy="12" r="3" />
                    ...
                    {error && (
                        <div style={{ marginTop: "14px", background: "var(--danger-bg)", border: "1px solid #fecaca", borderRadius: "8px", padding: "10px 14px", fontSize: "13px", color: "var(--danger)" }}>⚠️ {error}</div>
                    )}
                </div>
                <div style={{ padding: "14px 24px", borderTop: "1px solid #f1f0ec", display: "flex", justifyContent: "flex-end", gap: "10px" }}>
                    <button onClick={onClose} style={{ padding: "8px 20px", borderRadius: "8px", border: "1px solid #e4e2dd", background: "var(--bg-card)", fontSize: "13px", fontWeight: 500, cursor: "pointer", color: "var(--text-sub)" }}>Cancel</button>
                    <button onClick={handleSubmit} disabled={loading} style={{ padding: "8px 20px", borderRadius: "8px", border: "none", background: loading ? "var(--icon-muted)" : "var(--text-main)", color: "var(--bg-card)", fontSize: "13px", fontWeight: 600, cursor: loading ? "not-allowed" : "pointer" }}>
...
function PlaceModal({ place, onClose, onToggleStatus }) {
    ...
    return (
        <>
            <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
                <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--bg-card)", borderRadius: "16px", width: "100%", maxWidth: "480px", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
                    <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #f1f0ec", display: "flex", alignItems: "center", gap: "14px" }}>
                        <div style={{ width: "44px", height: "44px", borderRadius: "50%", background: "var(--text-main)", color: "var(--bg-card)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", fontWeight: 700, flexShrink: 0 }}>
                    ...
                    <div style={{ padding: "0 24px 16px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "10px" }}>
                        {[["⭐", typeof rating === "number" ? rating.toFixed(1) : rating, t("place_stat_rating")], ["💬", reviews, t("place_stat_reviews")], ["❤️", saves, t("place_stat_saves")], ["👁", visits, t("place_stat_visits")]].map(([icon, val, lbl]) => (
                            <div key={lbl} style={{ background: "var(--bg-surface)", borderRadius: "10px", padding: "12px 8px", textAlign: "center", border: "1px solid #f1f0ec" }}>
                                ...
                                <div style={{ fontSize: "11px", color: "var(--icon-muted)" }}>{lbl}</div>
                            </div>
                        ))}
                    </div>
                    <div style={{ padding: "14px 24px", borderTop: "1px solid #f1f0ec", display: "flex", justifyContent: "flex-end", gap: "10px" }}>
...
function ToggleBtn({ place, onToggle }) {
    ...
    return (
        <>
            <button onClick={() => setShowConfirm(true)} disabled={loading} style={{ padding: "5px 12px", borderRadius: "7px", border: "none", fontSize: "12px", fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", background: isActive ? "var(--danger-bg)" : "var(--success-bg)", color: isActive ? "var(--danger)" : "var(--success)", whiteSpace: "nowrap" }}>
    ...
    return (
        <div style={{ maxWidth: "1200px" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "1.5rem" }}>
                <div>
                    <h1 style={{ fontSize: "1.5rem", fontWeight: 700, color: "var(--text-main)", letterSpacing: "-0.02em", marginBottom: "0.25rem" }}> {t("places")} 🏪</h1>
            ...
            <div style={{ marginBottom: "1.25rem", display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
                ...
                {categories.length > 1 && (
                    <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} style={{ padding: "8px 12px", borderRadius: "8px", border: "1px solid #e4e2dd", fontSize: "13px", color: "var(--text-sub)", background: "var(--bg-card)", cursor: "pointer", outline: "none" }}>
            ...
            {/* Table */}
            {loading ? (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "200px", color: "var(--icon-muted)" }}>Loading...</div>
            ) : filtered.length === 0 ? (
                            ...
                            <thead>
                                <tr style={{ background: "var(--bg-surface)", borderBottom: "1px solid #e4e2dd" }}>
                                    {[t("id"), t("name"), t("category"), t("district"), t("rating"), t("status"), t("actions")].map((h) => (
                                        <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontSize: "11px", fontWeight: 600, color: "var(--text-sub)", textTransform: "uppercase", letterSpacing: "0.05em", whiteSpace: "nowrap" }}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {paginated.map((place, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td style={{ padding: "12px 14px", color: "var(--text-sub)" }}>{place.Category ?? "—"}</td>
                                            <td style={{ padding: "12px 14px", color: "var(--text-sub)" }}>{place.District ?? "—"}</td>
                                            <td style={{ padding: "12px 14px" }}>
